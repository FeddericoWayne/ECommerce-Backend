const router = require('express').Router();
const { Product, Category, Tag, ProductTag } = require('../../models');

// The `/api/products` endpoint

// get all products
router.get('/', async (req, res) => {
  
  // try-catch statement to retrieve all products with associated Category and Tag data
  try {
    const productData = await Product.findAll({
      include:[{model:Category},{model:Tag}]
    });
    // if no product is found
    if(!productData || productData.length ===0) {
      res.status(404).json({message:"Data Not Found!"});
      return;
    };
    // sends back product data with success code
    res.status(200).json(productData);

  // catches and sends back error  
  } catch(err) {

    res.status(400).json(err);
  }
});

// get one product
router.get('/:id', async (req, res) => {
  
  // try-catch statement to retrieve product data by id with associated Category and Tag data
  try {

    const productData = await Product.findByPk(req.params.id,{
      include:[{model:Category},{model:Tag}]
    });
    // if product is not found
    if (!productData || productData.length ===0) {
      res.status(404).json({message:"Product Not Found!"});
      return;
    };
    // sends back product data if found
    res.status(200).json(productData);

  // catches and returns error
  } catch(err) {

    res.status(400).json(err);
  }

});

// create new product
router.post('/', (req, res) => {
  /* req.body should look like this...
    {
      "product_name": "Basketball",
      "price": 200.00,
      "stock": 3,
      "category_id": [6],
      "tagIds": [1, 2, 3, 4] 
    }
  */
  Product.create(req.body)
    .then((product) => {
      // if there's product tags, we need to create pairings to bulk create in the ProductTag model
      if (req.body.tagIds.length) {
        const productTagIdArr = req.body.tagIds.map((tag_id) => {
          return {
            product_id: product.id,
            tag_id,
          };
        });
        return ProductTag.bulkCreate(productTagIdArr);
      }
      // if no product tags, just respond
      res.status(200).json({message: `Product Created: ${req.body.product_name.toLowerCase()}, Price: ${req.body.price}, Stock: ${req.body.stock}, Category_id: ${req.body.category_id}`});
    })
    .then(() => res.status(200).json({message: `Product Created: ${req.body.product_name.toLowerCase()}, Price: ${req.body.price}, Stock: ${req.body.stock}, Category_id: ${req.body.category_id}, Tag ID's: ${req.body.tagIds}`}))
    .catch((err) => {
      console.log(err);
      res.status(400).json(err);
    });
});

// update product
router.put('/:id', (req, res) => {
  // update product data
  Product.update(req.body, {
    where: {
      id: req.params.id,
    },
    individualHooks:true
  })
    .then((product) => {
      // find all associated tags from ProductTag
      return ProductTag.findAll({ where: { product_id: req.params.id } });
    })
    .then((productTags) => {
      // get list of current tag_ids
      const productTagIds = productTags.map(({ tag_id }) => tag_id);
      // create filtered list of new tag_ids
      const newProductTags = req.body.tagIds
        .filter((tag_id) => !productTagIds.includes(tag_id))
        .map((tag_id) => {
          return {
            product_id: req.params.id,
            tag_id,
          };
        });
      // figure out which ones to remove
      const productTagsToRemove = productTags
        .filter(({ tag_id }) => !req.body.tagIds.includes(tag_id))
        .map(({ id }) => id);

      // run both actions
      return Promise.all([
        ProductTag.destroy({ where: { id: productTagsToRemove } }),
        ProductTag.bulkCreate(newProductTags),
      ]);
    })
    .then((updatedProductTags) => res.status(200).json(updatedProductTags))
    .catch((err) => {

      res.status(400).json(err);
    });
});

router.delete('/:id', async (req, res) => {
  // delete one product by its `id` value
  try {

    const deletedProduct = await Product.findByPk(req.params.id);
    if (!deletedProduct || deletedProduct.length ===0) {
      res.status(404).json({message:"Product Not Found!"});
      return;
    };
    
    await Product.destroy({
      where:{
        id: req.params.id
      }
    });

    res.status(200).json({message:`Producted Deleted: ${deletedProduct.product_name}`});

  } catch(err) {

    res.status(400).json(err);
  }
});

module.exports = router;
